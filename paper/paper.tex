...
single-coil knee benchmark, roughly \SI{5}{\decibel} above a U-Net baseline.
...
produced under the new one. The revised system, its 212-test suite, and a
...
\SI{33.10}{\decibel} PSNR and \num{0.7274} SSIM, roughly \SI{5}{\decibel} above
the U-Net. Preparing that work for release, I audited the implementation against
its own documentation. The audit did not find the small discrepancies one
expects. It found that the artifact could not run at all, and that several
components which appeared to be working were not.
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{A reproducibility audit} of the released implementation,
        classifying 17 defects into those that prevent execution, those that
  ...
  \item \textbf{A verifiable artifact}: 212 automated tests including exactness
...
The suite comprises 212 tests running in roughly six seconds on CPU, organised
