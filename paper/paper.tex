...
This paper reports a systematic audit of my v1 implementation and a
substantially revised system. The audit found that the v1 code could not
...
In a controlled ablation on synthetic phantom data, the data-consistency
cascade beats a capacity-matched single-pass network by \SI{2.40}{\decibel}
on a U-Net backbone and \SI{2.16}{\decibel} on a SwinUNet backbone, both at
$p < 10^{-4}$ under a paired permutation test and both with fewer parameters
than the control. The near-equality of the two gains across architectures with
very different inductive biases indicates the improvement comes from the
physics constraint rather than the network. A secondary finding is that
complex-valued input on its own \emph{costs} \SI{2.14}{\decibel}, because the
magnitude loss leaves phase unsupervised --- which is precisely the freedom the
data-consistency projection removes. I do not restate the previously reported
fastMRI figures as validated results: they are not reproducible from the v1 release, and the corrections to the sampling and metric definitions
mean that numbers produced under the old protocol are not comparable to numbers
produced under the new one. The revised system, its 267-test suite, and a
...
\end{lstlisting}
No forward pass through the reported architecture was possible from the v1 code. A separate misspelled module filename (\code{reproductibility.py}, imported
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{A reproducibility audit} of my v1 implementation,
  ...
  \item \textbf{A verifiable artifact}: 267 automated tests at \SI{88}{\percent} statement
        coverage, including exactness
...
figures cannot be reproduced from the v1 code, and the corrections
...
is what my v1 implementation attempted but did not achieve.
...
This section documents the defects found in my v1 implementation. It is
...
quantity being reported depends on a training hyperparameter. In v1
...
\paragraph{Evidence in my v1 figures.} The training curves published with
v1 are reproduced in Figure~\ref{fig:prior-curves}, because they
...
\num{5e-5} and v1's ablation section reports that ``optimal
...
\caption{Training curves from v1: U-Net (top) and SwinUNet
...
v1's qualitative figure is reproduced as
...
own. The two cannot be separated without the v1 weights, and those were
never released.
...
\caption{Qualitative reconstructions from v1: zero-filled input
...
weights that produced them were never released and the v1 code cannot
...
The Class~1 defects establish that the v1 release cannot have produced
...
from v1. The panels show zero-filled reconstructions exhibiting the
...
maintained, and---unlike in v1---is actually used for validation,
...
physics-informed; it was not satisfied by my v1 implementation at any
...
\paragraph{Data consistency delivers, and the effect is
architecture-independent.} Against a capacity-matched control the cascade gains
\SI{+2.40}{\decibel} on the U-Net and \SI{+2.16}{\decibel} on the SwinUNet, both
at $p < 10^{-4}$ and both with \emph{fewer} parameters than the control they
beat. That two backbones with entirely different inductive biases---one
convolutional, one windowed-attention---produce gains within
\SI{0.24}{\decibel} of each other is the strongest evidence in this paper that
the improvement comes from the physics constraint rather than from the network.

\paragraph{Complex-valued input, on its own, is harmful.} Switching the U-Net
from magnitude to complex input costs \SI{2.14}{\decibel}
(\SI{36.23}{\decibel} to \SI{34.09}{\decibel}) at identical parameter count.
Doubling capacity recovers only \SI{0.46}{\decibel} of that.

I did not anticipate this, and the explanation is instructive. The loss is
computed on magnitude, so phase is entirely unsupervised. A complex-valued
network therefore optimises through a large null space: infinitely many complex
outputs share the same magnitude, and nothing in the objective distinguishes
them. The network must discover a phase convention on its own, and gradient
descent through that degeneracy is simply harder than mapping magnitude to
magnitude directly.

This makes the data-consistency result more interesting rather than less. DC is
the only component here that constrains phase, because it operates in $k$-space
where phase is not optional. It does not merely recover the
\SI{2.14}{\decibel} that complex representation costs---it recovers all of it
and adds \SI{0.72}{\decibel} beyond the magnitude-only baseline. The
constraint is doing real work, and it is doing it precisely where the loss
function cannot reach.

\paragraph{The SwinUNet arm is under-trained, and I report it as such.} The
SwinUNet sits roughly \SI{6}{\decibel} below the U-Net at every rung of the
ablation. This is a statement about the experiment's scale, not about the
architecture. At 0.21\,M parameters it has less than half the
U-Net's capacity, and 240 synthetic training slices is far too little for a
transformer trained from scratch with no pretraining---transformers lack the
locality prior that lets a convolutional network learn from small datasets.
The correct reading is that the ablation establishes the DC effect on both
backbones and establishes nothing about their relative merits. Ranking the two
from this experiment would repeat, in a new form, exactly the error of drawing
architectural conclusions from an inadequately controlled comparison.
...
The suite comprises 267 tests covering \SI{88}{\percent} of statements and
running in under a minute on CPU, organised
so that each defect found in v1 has a corresponding regression test.
Notable coverage:
...
test in my v1 suite passed, because none of them ever loaded a file, and
...
that without it nothing in my v1 repository could be exercised end to
...
improvement that are easily conflated, and they turn out to point in different
directions. Complex-valued input alone costs \SI{2.14}{\decibel}. Doubling
capacity returns \SI{0.46}{\decibel} of that. Data consistency returns the
remainder and adds \SI{0.72}{\decibel} beyond the magnitude-only baseline.

Reported only against that baseline, the cascade would have looked like a
modest \SI{+0.72}{\decibel} improvement---and that framing would have concealed
both that complex representation is a substantial handicap and that the physics
constraint more than overcomes it. The capacity-matched control is what makes
the \SI{2.40}{\decibel} attributable to data consistency rather than to
parameters. A cascade of $T$ stages carries $T$ times the parameters of its
backbone, so omitting that control would have repeated, in subtler form, the
error this paper set out to correct.

That the same constraint yields \SI{+2.40}{\decibel} on a convolutional
backbone and \SI{+2.16}{\decibel} on a windowed-attention one is the result I
would most want replicated. Two architectures with very different inductive
biases should not respond so similarly to an architectural intervention; they
should respond similarly to a constraint on the solution space, which is what
data consistency is.
...
coefficients to $8.3\times10^{-7}$, verified automatically on every commit.

In a controlled ablation against capacity-matched controls, that constraint is
worth \SI{2.40}{\decibel} on a U-Net backbone and \SI{2.16}{\decibel} on a
SwinUNet backbone---both at $p < 10^{-4}$, and both achieved with fewer
parameters than the single-pass networks they beat. The near-equality of those
two figures, across architectures whose inductive biases have almost nothing in
common, is the clearest indication that what helps is the constraint on the
solution space rather than the choice of network. A secondary and initially
unwelcome finding is that complex-valued input on its own \emph{costs}
\SI{2.14}{\decibel}: the magnitude loss leaves phase unsupervised, and the
network must optimise through that degeneracy. Data consistency is the only
component here that reaches phase, and it recovers the deficit and more.

I deliberately do not restate the earlier benchmark figures as validated
results. They are not reproducible from the v1 release, and the
