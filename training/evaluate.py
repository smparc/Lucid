...
from training.train  import CombinedLoss, SSIMLoss, psnr as compute_psnr, ssim_metric
...
def load_model(model_name: str, ckpt_path: str, device: torch.device) -> torch.nn.Module:
    ...
    print(f"  Loaded {model_name} from {ckpt_path}  (epoch {ckpt['epoch']}, "
          f"val_loss={ckpt['val_loss']:.4f}, PSNR={ckpt['val_psnr']:.2f}dB)")
    return model
